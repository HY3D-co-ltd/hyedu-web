...
export function generateStaticParams() {
  const locales = ['ko', 'en'];
  return locales.flatMap((locale) => eventPosts.map((p) => ({ locale, id: p.id })));
}
    ...
    <>
      <BreadcrumbJsonLd
        items={[
          ...
          { name: detail.title, href: `/${locale}/board/events/${id}` },
        ]}
      />
      ...
      <nav className="max-w-4xl mx-auto px-4 pb-16 flex gap-3">
        {prev ? (
          <Link
            href={`/${locale}/board/events/${prev.id}`}
        ...
        {next ? (
          <Link
            href={`/${locale}/board/events/${next.id}`}
